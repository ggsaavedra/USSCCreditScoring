  ...
  res.amount <- rbind(
    ...
    c(seg$size[5], mean(c5$frequency), mean(c5$mean_amount), (mean(c5$standard_deviation)/mean(c5$mean_amount))*100, mean(c5$minimum), mean(c5$maximum), mean(c5$less_points), mean(c5$plus_points), mean(c5$age))
  ...
  prov <- as.data.frame(table(c5$province))
  prov$percent <- round((prov$Freq/sum(prov$Freq))*100)
  provc5 <- (prov[order(prov$Freq, decreasing = TRUE),])[1:4,]
  ...
  pr <- as.data.frame(cbind(paste0(provc1$Var1 , ' (', provc1$percent, '%)'),
                            ...
                            paste0(provc5$Var1 , ' (', provc5$percent, '%)')))
  rownames(pr) <- c('Province 1', 'Province 2', 'Province 3', 'Province 4', 'Province 5')
  res <- rbind(res, pr)
  names(res) <- c('A', 'B', 'C', 'D', 'E')
